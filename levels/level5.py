...
class Level5(BaseLevel):
    def description(self):
        return "第五关：函数\n任务：定义一个函数greet(name)，该函数接收一个参数name，并打印出'你好, name！'。然后调用这个函数。"
    ...
    def check_code(self, code):
        ...
        try:
            with contextlib.redirect_stdout(output):
                local_namespace['greet'](test_name)
        except Exception as e:
            return False, f"代码运行错误: {e}"

        printed_output = output.getvalue().strip()
        expected_output = f"你好, {test_name}！"
    ...
    def teaching(self):
        return ("函数用于将一段代码封装成一个可重复使用的代码块。使用 def 关键字来定义函数。例如：\n"
                "def greet(name):\n"
                "    print(f'你好, {name}！')\n"
                "这段代码定义了一个名为 greet 的函数，它接收一个参数 name，并打印出'你好, name！'。")
